...
A folder rule applies a time to live to every file inside a folder, at any depth, including
attachments and other non-Markdown files. Each rule sets its own TTL, action, destination and ignore
patterns.
...
files_ preference — system trash, the vault's `.trash` folder, or permanent deletion.
