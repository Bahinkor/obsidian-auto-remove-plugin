...
export class AutoRemoveSettingTab extends PluginSettingTab {
      ...
      .addText((text) => {
        text
          ...
          .onChange((value) => {
            ...
            if (Number.isInteger(days) && days >= 0) void this.store.update({ defaultTtlDays: days });
          });
      ...
      new Setting(this.containerEl)
        ...
        .addSearch((search) => {
          ...
          new FolderSuggest(this.app, search.inputEl, (folder) =>
            void this.store.update({ defaultMoveDestination: folder }),
          );
        });
    }
  }
  ...
  private async deleteRule(id: string): Promise<void> {
    await this.replaceRules(
      this.store.settings.folderRules.filter((rule) => rule.id !== id),
    );
    this.display();
  }
