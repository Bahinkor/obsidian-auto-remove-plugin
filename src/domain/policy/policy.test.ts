...
describe('FolderRulePolicySource', () => {
  it('claims files inside the configured folder', () => {
    ...
    expect(policy?.origin).toEqual({ source: 'folder-rule', ruleId: 'rule-Inbox', folder: 'Inbox' });
  });
  ...
  it('excludes paths matched by the rule ignore patterns', () => {
    const source = folderRules([rule({ folder: 'Inbox', ignorePatterns: ['Templates/**', '*.pdf'] })]);
  ...
  it('anchors a pattern containing a slash, exactly as gitignore does', () => {
    ...
    const everywhere = folderRules([rule({ folder: 'Inbox', ignorePatterns: ['**/Templates/**'] })]);
...
describe('PolicyResolver', () => {
  ...
  it('lets an opt-in override the folder ignore patterns that would exclude it', () => {
    ...
    expect(resolver.resolve(markdown('Templates/keep.md', { 'auto-remove': true }))?.ttlDays).toBe(7);
  });
  ...
  it('lets an explicit opt-out veto the folder rule covering it', () => {
    ...
    expect(resolver.resolve(markdown('Inbox/note.md', { 'auto-remove': false, ttl: 1 }))).toBeNull();
  });
